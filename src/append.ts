...
import { COLUMNS, canonicalKeyParts, deriveAgentId, encodeCsv, factFingerprint, generateId, readRecords, shardPath, utcTimestamp, validateRecord } from "./records.ts";
...
export type AppendFact = { subject: string; predicate: string; object: string; confidence?: number; evidence?: string; supersedes?: string; tags?: string };
export type AppendRequest = { sessionId: string; kind: string; summary: string; source: string; evidence?: string; tags?: string; facts: AppendFact[] };
...
function rowLimit(env: NodeJS.ProcessEnv): number { const raw = env.PI_KNOWLEDGE_SHARD_ROW_LIMIT; if (raw === undefined) return 10000; const n = Number(raw); if (!Number.isInteger(n) || n < 1 || n > 1_000_000) throw new Error("PI_KNOWLEDGE_SHARD_ROW_LIMIT must be an integer from 1 to 1000000"); return n; }
...
function validateRequest(request: AppendRequest) {
  ...
  const episodeValues = [
    ...
    validateText(request.kind, "kind"),
    ...
    validateText(request.tags, "tags", { optional: true }),
  ];
  ...
  for (const fact of request.facts) {
    ...
    const subject = validateText(fact.subject, "subject", { maxBytes: MAX_KEY_BYTES }), predicate = validateText(fact.predicate, "predicate", { maxBytes: MAX_KEY_BYTES }), object = validateText(fact.object, "object", { maxBytes: MAX_KEY_BYTES });
    const evidence = validateText(fact.evidence, "evidence", { optional: true, multiline: true, maxBytes: MAX_TEXT_BYTES }), supersedes = validateText(fact.supersedes, "supersedes", { optional: true }), tags = validateText(fact.tags, "tags", { optional: true });
    if (fact.confidence !== undefined && (typeof fact.confidence !== "number" || !Number.isFinite(fact.confidence) || fact.confidence < 0 || fact.confidence > 1)) throw new Error("confidence must be a number between 0 and 1");
    canonicalKeyParts(subject); canonicalKeyParts(object); factFingerprint(subject, predicate, object); secretValues.push(subject, predicate, object, evidence, supersedes, tags);
  }
...
function exactRetry(request: AppendRequest, agent: string, facts: CsvRecord[], episodes: CsvRecord[]) { const episode = episodes.find(e => e.episode_id === facts[0].episode_id); if (!episode || facts.some(f => f.episode_id !== episode.episode_id)) return false; if (episode.agent_id !== agent || episode.session_id !== request.sessionId || episode.kind !== request.kind || episode.summary !== request.summary || episode.source !== request.source || episode.evidence !== (request.evidence ?? "") || episode.tags !== (request.tags ?? "")) return false; return request.facts.every((input, i) => { const fact = facts[i]; return fact.subject === input.subject && fact.predicate === input.predicate && fact.object === input.object && fact.confidence === String(input.confidence ?? 1) && fact.evidence === (input.evidence ?? "") && fact.supersedes === (input.supersedes ?? "") && fact.tags === (input.tags ?? ""); }); }
...
export async function appendKnowledge(root: string, request: AppendRequest, options: AppendOptions = {}): Promise<AppendResult> {
  ...
  try {
    ...
    const episodeId = generateId("episodes", ids); ids.add(episodeId); const episode: CsvRecord = { schema_version: "1", episode_id: episodeId, created_at: timestamp, agent_id: agentId, session_id: request.sessionId, kind: request.kind, summary: request.summary, source: request.source, evidence: request.evidence ?? "", tags: request.tags ?? "" }; validateRecord("episodes", episode);
    const facts = request.facts.map(input => { const fact_id = generateId("facts", ids); ids.add(fact_id); const row: CsvRecord = { schema_version: "1", fact_id, subject: input.subject, predicate: input.predicate, object: input.object, episode_id: episodeId, created_at: timestamp, confidence: String(input.confidence ?? 1), evidence: input.evidence ?? "", supersedes: input.supersedes ?? "", tags: input.tags ?? "", fingerprint: factFingerprint(input.subject, input.predicate, input.object) }; validateRecord("facts", row); return row; });
