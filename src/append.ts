...
export type AppendFact = { subject: string; predicate: string; object: string; evidence?: string; supersedes?: string; tags?: string };
...
export async function appendKnowledge(root: string, request: AppendRequest, options: AppendOptions = {}): Promise<AppendResult> {
  ...
  try {
    ...
    const facts = request.facts.map(input => { const fact_id = generateId("facts", ids); ids.add(fact_id); const row: CsvRecord = { schema_version: "1", fact_id, subject: input.subject, predicate: input.predicate, object: input.object, episode_id: episodeId, created_at: timestamp, evidence: input.evidence ?? "", supersedes: input.supersedes ?? "", tags: input.tags ?? "", fingerprint: factFingerprint(input.subject, input.predicate, input.object) }; validateRecord("facts", row); return row; });
